...
# SPECIFICATION: Utilize random forest and bagging techniques to predict pattern of handwritten digits based on a 32x32 bitsmap
...
with open('optdigits.tes', 'r') as testingFile:
  ...
  for k in range(20): #we will create 20 bootstrap samples here (k = 20). One classifier will be created for each bootstrap sample
      ...
      for rowIndex,row in enumerate(bootstrapSample):
          size = len(row)
          temp = []
      ...
      for i,testSample in enumerate(dbTest):
          ...
          classVotes[i][classPredictedInt] = classVotes[i][classPredictedInt] +1
  ...
  #--> add your Python code here
  accuracy = 0
  for index,element in enumerate(classVotes):
      maxValue = max(element)
      indexOfMaxValue = element.index(maxValue)
      currentSample = dbTest[index]
      if indexOfMaxValue == int(currentSample[len(currentSample)-1]):
          accuracy = accuracy + 1
